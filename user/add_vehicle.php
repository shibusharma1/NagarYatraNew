    ...
    if (empty($errors)) {
        ...
        if (move_uploaded_file($_FILES['thumb_image']['tmp_name'], $thumb_image_target) && move_uploaded_file($_FILES['bill_book_image']['tmp_name'], $bill_book_image_target)) {
            ...
            if ($stmt->execute()) {
                ...
                $updateUserQuery = "UPDATE user SET vehicle_id = ?, role = 1 WHERE id = ?";
                $stmt = $conn->prepare($updateUserQuery);
                $stmt->bind_param("ii", $lastInsertedVehicleId, $_SESSION['id']);
...
<form class="vehicle-form-style" action="" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <input type="text" class="form-input" id="chassis_number" name="chassis_number" placeholder="Enter your vehicle chassis number" />
