...
if (!empty($validBookingIds)) {
    ...
    $countQuery = "SELECT COUNT(*) AS total FROM booking WHERE id IN ($ids) AND is_delete = 0 AND status = 3";
    $countResult = $conn->query($countQuery);
    $totalRecords = $countResult->fetch_assoc()['total'];
    $totalPages = ceil($totalRecords / $limit);

    // Fetch the paginated valid bookings
    $sql = "SELECT * FROM booking WHERE id IN ($ids) AND is_delete = 0 AND status = 3 ORDER BY created_at DESC LIMIT $limit OFFSET $offset";
    $result = $conn->query($sql);

    echo '<table>';
    echo '<h2>Approved Ride</h2>';
    echo '<tr><th>S.N</th><th>Pickup Location</th><th>Destination Location</th><th>Estimated Km</th><th>Estimated duration</th><th>Estimated cost</th><th>OTP</th><th>Map</th><th>Action</th></tr>';
    if ($result && $result->num_rows > 0) {
        ...
        <br>
            ...
            echo '<td> <form method="POST" action="approve_reject_ride.php">
                                    ...
                                    <button type="submit" class="btn btn-danger" style="padding: 5px 10px; border-radius: 12px; font-size: 14px; font-weight: 600; color: #fff; background-color: green; margin-bottom:3px;">Complete</button>
                                </form>
